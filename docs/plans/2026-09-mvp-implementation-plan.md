...
> **Progress:** M0–M4 built, tested and reviewed (2026-09-11); M5 built and tested (2026-09-18); M6–M9 not started. Per-milestone detail in §17.
>
...
| `/webhooks/resend` | hooks | POST | Raw body; verifies the `svix-*` signature (HMAC-SHA256, 5-minute window, any of the `v1,` signatures); `email.bounced` (Permanent) / `email.complained` → `UPDATE subscribers SET blocked_at = COALESCE(blocked_at, now()), blocked_reason = CASE WHEN blocked_reason = 'complaint' THEN 'complaint' ELSE :reason END WHERE email = :email` — a complaint replaces a bounce reason and is never replaced, because a bounce block is lifted by the next confirmation; every other event ignored. No dedup table: writes are set-to-value, so at-least-once delivery and dashboard replays are no-ops. A bad signature gets 401, so a wrong secret shows up as failures in the Resend dashboard and is retried, instead of every bounce vanishing; everything signed gets 200. |
...
- **Webhooks**: Resend signature on raw bytes with constant-time compare, 5-minute replay window; every write is set-to-value, so replays are harmless; 401 on a bad signature, 200 for everything signed.
...
- **Status: built and tested, acceptance open, 2026-09-18.** 53 new tests (408 in total) cover every rule in §10 for the three routes, the webhook (vectors, replay, transient bounce, complaint after bounce, forged signature) and the cleanup, including the confirmation page's rule for which summary it may link. The whole flow was clicked through in Chrome against `docker compose` — sign-up with the HTMX swap, confirm, unsubscribe page and POST. That run found two bugs the tests had not: every `logger.exception` crashed in console format, and the unsubscribe pages carried neither the creator's name nor colour; both fixed test-first (`implementation-notes.html`). **Sending is verified live:** on 2026-09-18 a sign-up through the running app sent a real confirm mail via Resend to its test address `delivered@resend.dev` (status *delivered*), from `onboarding@resend.dev` via the new `SENDER_ADDRESS` override (no verified domain before M9). **Not verified: the webhook.** No event has arrived from Resend yet — the development machine sits in a network that blocks the `cloudflared` tunnel, and a tunnel from that machine was ruled out, so the live bounce test moved to M9, where the app has a public address. Until then the payload handling rests on Resend's documented example, not on a real event.

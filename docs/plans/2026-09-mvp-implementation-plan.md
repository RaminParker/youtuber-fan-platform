...
| `/webhooks/resend` | hooks | POST | Raw body; verifies the `svix-*` signature (HMAC-SHA256, 5-minute window, any of the `v1,` signatures); `email.bounced` (Permanent) / `email.complained` → `UPDATE subscribers SET blocked_at = COALESCE(blocked_at, now()), blocked_reason = CASE WHEN blocked_reason = 'complaint' THEN 'complaint' ELSE :reason END WHERE email = :email` — a complaint replaces a bounce reason and is never replaced, because a bounce block is lifted by the next confirmation; every other event ignored. No dedup table: writes are set-to-value, so at-least-once delivery and dashboard replays are no-ops. Always 200. |
...
- **Status: built and tested, acceptance open, 2026-09-18.** 53 new tests (408 in total) cover every rule in §10 for the three routes, the webhook (vectors, replay, transient bounce, complaint after bounce, forged signature) and the cleanup, including the confirmation page's rule for which summary it may link. The whole flow was clicked through in Chrome against `docker compose` — sign-up with the HTMX swap, confirm, unsubscribe page and POST. That run found two bugs the tests had not: every `logger.exception` crashed in console format, and the unsubscribe pages carried neither the creator's name nor colour; both fixed test-first (`implementation-notes.html`). **Not verified:** no mail has left through Resend, and no webhook has arrived from it — `RESEND_API_KEY` and `RESEND_WEBHOOK_SECRET` are still empty. Until the owner has run the acceptance (a real confirm mail to the Resend account's own address, then a test send to `bounced@resend.dev` through a `cloudflared` tunnel), the webhook's payload handling rests on Resend's documented example, not on a real event. `SENDER_ADDRESS` is new for exactly this run (no verified domain before M9).
